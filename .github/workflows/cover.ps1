...
$OUTPUT_DIR_NAME="$DATE-$Env:SUITE-$Env:DLL-$Env:NAMESPACE$Env:CLASS$Env:METHOD-$Env:GITHUB_WORKFLOW-$Env:GITHUB_RUN_NUMBER"
...
foreach ($s in $Env:SEARCHERS.Split(" "))
{
    ...
    foreach ($t in $Env:TIMEOUTS.Split(" "))
    {
        ...
        # TODO: Disable verbosity

        if ($Env:NAMESPACE) 
        {
            dotnet VSharp.Runner.dll --namespace "$Env:NAMESPACE" "$DLL_PATH" -t "$t" -o "$TEMP_DIR" --strat "$s"
        } 
        elseif ($Env:METHOD) 
        {
            dotnet VSharp.Runner.dll --method "$Env:METHOD" "$DLL_PATH" -t "$t" -o "$TEMP_DIR" --strat "$s"
        } 
        elseif ($Env:CLASS) 
        {
            dotnet VSharp.Runner.dll --public-methods-of-class "$Env:CLASS" "$DLL_PATH" -t "$t" -o "$TEMP_DIR" --strat "$s"
        } 
        else 
        {
            dotnet VSharp.Runner.dll --all-public-methods "$DLL_PATH" -t "$t" -o "$TEMP_DIR" --strat "$s"
        } 
