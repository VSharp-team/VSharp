...
class StateGNNEncoderConvEdgeAttrGraphConvAll(torch.nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict, edge_attr=None, edge_type=None):
        game_x = self.conv1(
            x_dict['game_vertex'],
...
class StateModelEncoder(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels):
        super().__init__()
        self.state_encoder = StateGNNEncoderConvEdgeAttrGraphConvAll(hidden_channels, out_channels)

    def forward(self, x_dict, edge_index_dict, edge_attr=None):
        z_dict = {}
        z_dict['state_vertex'] = self.state_encoder(x_dict, edge_index_dict, edge_attr, edge_type)
